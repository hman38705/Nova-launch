...
router.get('/', authenticateAdmin, auditLog('list_tokens', 'token'), async (req, res) => {
  try {
    ...
    if (filters.flagged !== undefined) {
      tokens = tokens.filter(t => t.flagged === (filters.flagged === 'true'));
    }
    if (filters.creator) {
...
router.get('/:id', authenticateAdmin, auditLog('view_token', 'token'), async (req, res) => {
  try {
...
router.patch('/:id', authenticateAdmin, auditLog('update_token', 'token'), async (req, res) => {
  try {
...
router.delete('/:id', authenticateAdmin, requireSuperAdmin, auditLog('delete_token', 'token'), async (req, res) => {
  try {

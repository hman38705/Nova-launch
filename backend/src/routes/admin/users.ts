...
router.get('/', authenticateAdmin, auditLog('list_users', 'user'), async (req, res) => {
  try {
...
router.get('/:id', authenticateAdmin, auditLog('view_user', 'user'), async (req, res) => {
  try {
    ...
    const userTokens = allTokens.filter(t => t.creatorAddress === user.address);

    // Get user's audit logs (if they're an admin)
    const auditLogs = user.role !== 'user' 
    ...
    res.json({
      ...
      activity: {
        ...
        totalBurned: userTokens.reduce((sum, t) => sum + BigInt(t.burned || '0'), BigInt(0)).toString(),
...
router.patch('/:id', authenticateAdmin, requireSuperAdmin, auditLog('update_user', 'user'), async (req, res) => {
  try {
...
router.get('/:id/export', authenticateAdmin, auditLog('export_user_data', 'user'), async (req, res) => {
  try {
    const user = await Database.findUserById(req.params.id);
    ...
    res.setHeader('Content-Disposition', `attachment; filename="user_${user.id}_export.json"`);
    res.json(exportData);
  } catch (error) {

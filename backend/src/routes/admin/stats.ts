...
router.get('/', authenticateAdmin, auditLog('view_stats', 'stats'), async (req, res) => {
  try {
    ...
    const totalVolumeBurned = tokens.reduce((sum, t) => {
    ...
    const revenueGenerated = (BigInt(totalVolumeBurned) * BigInt(5) / BigInt(1000)).toString();
    ...
    const weeklyTokens = tokens.filter(t => t.createdAt >= oneWeekAgo).length;
    const monthlyTokens = tokens.filter(t => t.createdAt >= oneMonthAgo).length;
    ...
    const monthlyUsers = users.filter(u => u.createdAt >= oneMonthAgo).length;
